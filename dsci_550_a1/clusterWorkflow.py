...
def main():
    """
        Main clustering workflow using ettlib and tika-similarity. Will unpack dataset into jsons if not done already
        Inputs:

            [None] | User inputs are collective interactively
                - method        | Clustering method. Options: '(-j)accard', '(-c)osine', '(-e)dit'
                - folderName    | Folder name to shove all the outputs into
                - num_files     | determines sample of dataset used. Either [int] or [str]
                    - [int]  | size of random sample. (generated in clusterHelper.py) 
                    - [str]  | filepath to .txt specifying exact indicies to sample. 
                    
        Outputs:
            [json files]         | Outputs from tsv2json (Credit: https://github.com/chrismattmann/etllib)    
                - ./data/tika_similarity/**  
    
            [Clustering files]
                - ./clustering/[method]/visualization/clusters.json, 
                - ./clustering/[method]/visualization/levelCluster.json
                - ./clustering/[method]/visualization/circles.json, 
                - ./clustering/[method]/visualization/circlpacking.html, 
                - ./clustering/[method]/visualization/cluster-d3.html
                - ./clustering/[method]/visualization/levelCluster-d3.html, 

            [Metadata file]
                - ./clustering/{method}/{folderChoice}/metadata.json
                    - "number of files"             | number of indicies clustered
                    - "haunted_places_indicies"     | indicies used in haunted_places_cleaned.tab if user put custom indicies. Else "random sample"
                    - "fields"                      | features used to cluster
        ...
        NOTES:
            ...
            - tsv2josn step is skipped if ./data/tika_similarity/** already exists and has jsons in it.    
    eg:
    ```bash
    >>> python ./dsci_550_a1/clusterWorkflow.py
    ```

    eg output:
    ```
    Enter a clustering type:
         -j [jaccard] 
         -e [edit-distance] 
         -c [cosine]
    --------------------------------------------------
    ...
    All results and metadata will be saved to this Choice './clustering/jaccard/[Choice]'
    --------------------------------------------------
    >>> Choice: folderName

    --------------------------------------------------
    Enter number of haunted places to cluster:
    Type "-1" to input a list of custom indices
    --------------------------------------------------
    >>> Choice: -1

    Enter the relative filepath to a '.txt' file specifying the indices you want from 
    "haunted_places_features_added.tab" (e.g., './placesIWant.txt'):
    --------------------------------------------------
    >>> Choice: ./file.txt

    --------------------------------------------------
    You have already unpacked the haunted place JSON files
    Total JSON count: 10991
    Skipping unpacking using 'tsvtojson' and 'repackage.py'.
    --------------------------------------------------

    Created 500 JSON copies in ./data/tika_similarity/temp
    Now running './clones/tika-img-similarity/tikasimilarity/distance/jaccard_similarity.py'

    --------------------------------------------------
    Metadata saved to ./clustering/cosine/{folderPath}/metadata.json

    Moving output files to ./clustering/cosine/{folderPath}/visualization
    ...
    # User selects a random sample of indicies or specifies custom indicies in a .txt

    num_files = '' 
    while not num_files.isdigit() or not (0 < int(num_files) <= 10991):
        ...
        if num_files == '-1': 
            ...
            while True: 
                file_name = input("Enter the relative filepath to a '.txt' file specifying the indicies you want from \"haunted_places_features_added.tab\" (e.g., './placesIWant.txt'):  \n" + '-'*50 + "\n" + "Choice: ").strip()
                
                # open.txt and check if list is in valid format
                try: 
                    ...
                    if isinstance(num_files_check, list) and all ((isinstance(num, int) and 0 <= num <= 10991) for num in num_files_check):
                        ...
                        print(f"Make sure all the entries in the list in {file_name} are integers between [0,10991]")
                except json.JSONDecodeError:
                    print(f"Make sure the list in {file_name} is in proper json list format  (e.g., '[1, 2, 3]')")
                except FileNotFoundError:
                    print(f"Error: {file_name} is not a valid filepath. Make sure you input a valid .txt path")
            break


    #########################################################
    ## Field Selection ##
    # User selects what fields they want to cluster by
    ...
    # If num_files is a list of integers, record its length. Else num_files is an integer equal to the size of random cluster sample
    if len(num_files.split(',')) > 1:
        number_of_files = len(num_files.split(','))
    else:
        number_of_files = num_files
    metadata = {
        "number of files" : number_of_files,
        "haunted place indicies" : num_files if not num_files.strip().isdigit() else 'random sample', 
        "fields" : fields,
        "input_data" : outfile, 
